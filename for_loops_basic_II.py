# Biggie Size - Given a list, write a function that changes all 
# positive numbers in the list to "big".

#     Example: biggie_size([-1, 3, 5, -5]) returns that same list, b
# ut whose values are now [-1, "big", "big", -5]

# arr=[1, -3 , -5, 7 , 2]
# def biggie_size(arr):
#     for i in range(len(arr)):
#         if arr[i] > 0:
#             arr[i] = 'BIGLY'
#     return(arr)
# print(biggie_size(arr))

# ==============================

# Count Positives - Given a list of numbers, create a function to replace the last
#  value with the number of positive values. (Note that zero is not considered to be a positive number).

#     Example: count_positives([-1,1,1,1]) changes the original list to [-1,1,1,3] and returns it
#     Example: count_positives([1,6,-4,-2,-7,-2]) changes the list to [1,6,-4,-2,-7,2] and returns it


# arr=[-1, 3, 5, -2, -3]
# def count_positives(arr):
#     count = 0
#     for i in range(len(arr)):
#         if arr[i] > 0:
#             count += 1
#     arr[-1] = count
#     return arr
# print(count_positives(arr))

# ===========================================

# Sum Total - Create a function that takes a list and returns the sum of all the values in the list.

#     Example: sum_total([1,2,3,4]) should return 10
#     Example: sum_total([6,3,-2]) should return 7

# arr=[1,2,3,4,5]
# def sum_total(arr):
#     count = 0
#     for i in range(len(arr)):
#         count = count + arr[i]
#     return count
# print(sum_total(arr))

# =====================================

# Average - Create a function that takes a list and returns the average of all the values.x

#     Example: average([1,2,3,4]) should return 2.5

# arr=[1,2,3,4]
# def average(arr):
#     sum = 0
#     for i in range(len(arr)):
#         sum = sum + arr[i]
#     return sum / len(arr)
# print(average(arr))

# =======================

# Length - Create a function that takes a list and returns the length of the list.

#     Example: length([37,2,1,-9]) should return 4
#     Example: length([]) should return 0

# arr = [1,2,3,4,5,6]
# def length(arr):
#     return(len(arr))
# print(len(arr))

# ==========================

# Minimum - Create a function that takes a list of numbers and returns the minimum 
# value in the list. If the list is empty, have the function return False.

#     Example: minimum([37,2,1,-9]) should return -9
#     Example: minimum([]) should return False

# arr=[3,12,5,1,-2]
# def minimum(arr):
#     min = arr[0]
#     if len(arr) == 0:
#         return False
#     for i in arr:
#         if i < min:
#             min = i
#     return min
# print(minimum(arr))

# ==========================

# Maximum - Create a function that takes a list and returns the maximum value in the
#  list. If the list is empty, have the function return False.

#     Example: maximum([37,2,1,-9]) should return 37
#     Example: maximum([]) should return False


# arr=[1,3,5,12,7,9]
# def maximum(arr):
#     max = arr[0]
#     if len(arr) == 0:
#         return False
#     for i in arr:
#         if i > max:
#             max = i
#     return max
# print(maximum(arr))


# =====================================

# Ultimate Analysis - Create a function that takes a list and returns a dictionary 
# that has the sumTotal, average, minimum, maximum and length of the list.

#     Example: ultimate_analysis([37,2,1,-9]) should return {'sumTotal': 31, 'average': 7.75, 'minimum': -9, 'maximum': 37, 'length': 4 }

# ????????
# ========================

# Reverse List - Create a function that takes a list and return that list with values 
# reversed. Do this without creating a second list. (This challenge is known to appear during basic technical interviews.)

#     Example: reverse_list([37,2,1,-9]) should return [-9,1,2,37]


# arr=[1,3,5,7]
# def reversed(arr):
#     arr = arr[::-1]
#     return arr
# print(reversed(arr))
















































































































